...
golden="$project_root/tests/golden/subset-bed-thin-counts.frq.count"
...
work_dir=$(mktemp -d "$project_root/tests/output/thin-benchmark.XXXXXX")
trap 'rm -rf -- "$work_dir"' EXIT

for threads in 8 16; do
    prefix="$work_dir/t$threads"
    timing="$result_dir/subset-bed-thin-counts-t$threads.time.txt"
    /usr/bin/time \
        -f $'wall_seconds=%e\nuser_seconds=%U\nsystem_seconds=%S\ncpu=%P\nmax_rss_kb=%M' \
        -o "$timing" \
        "$ng_binary" \
    ...
    printf 'threads=%d: PASS (byte-identical BED + thin counts)\n' \
        "$threads"
    cat "$timing"
